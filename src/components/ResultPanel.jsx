...
const ResultPanel = ({ seqTime, parTime, smartTime, operation }) => {
  // Check which results are available
  const seqAvailable = seqTime !== null && seqTime !== undefined;
  const parAvailable = parTime !== null && parTime !== undefined;
  const smartAvailable = smartTime !== null && smartTime !== undefined;
  
  const bothResultsAvailable = seqAvailable && parAvailable;
  ...
  const fastestMethod = (() => {
    ...
    if (seqAvailable) availableTimes.push({ time: seqTime, method: 'sequential' });
    if (parAvailable) availableTimes.push({ time: parTime, method: 'parallel' });
    if (smartAvailable) availableTimes.push({ time: smartTime, method: 'smart' });
    
    if (availableTimes.length === 0) return null;
    
    const fastest = availableTimes.reduce((prev, current) => 
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-8 border border-gray-200 dark:border-gray-700">
      <div className="flex items-center justify-between mb-6">
        ...
        {fastestMethod && (
          ...
          >
            {fastestMethod === 'parallel' ? 'Paralel Menang!' : 
             fastestMethod === 'smart' ? 'Smart Menang!' : 'Berurutan Menang!'}
          </div>
        )}
      </div>

      {(bothResultsAvailable || smartAvailable) ? (
        <>
          <div className="mb-6">
            {seqAvailable && (
              <>
            ...
            {smartAvailable && (
              <>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-medium text-orange-600 dark:text-orange-400">Smart</span>
                  <span className="text-sm font-medium text-black dark:text-white">{formatWaktu(smartTime)}</span>
                </div>
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4">
                  <div
                    className={`h-4 rounded-full ${fastestMethod === 'smart' ? 'bg-orange-600' : 'bg-gray-600'}`}
                    style={{ 
                      width: seqAvailable 
                        ? `${Math.min((smartTime / seqTime) * 100, 100)}%` 
                        : '100%' 
                    }}
                  ></div>
                </div>
          ...
          <div className="bg-orange-50 dark:bg-orange-900/20 rounded-lg p-5 border border-orange-200 dark:border-orange-800">
            ...
            <div className="mt-4">
              <p className="text-3xl font-bold text-orange-600 dark:text-orange-400">
                {formatWaktu(smartTime)}
              </p>
              <p className="text-sm text-orange-700 dark:text-orange-300 mt-1">
                Waktu pemrosesan otomatis terbaik
              </p>
            </div>
          </div>
        )}
      ...
      <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border border-indigo-200 dark:border-indigo-800">
        ...
        <p className="text-sm text-indigo-700 dark:text-indigo-300">
          ...
          {smartAvailable && (
            ` Mode Smart memilih pendekatan terbaik secara otomatis berdasarkan ukuran data dan karakteristik beban kerja.`
          )}
        </p>
      </div>
    </div>
